...
export function AskAI() {
  ...
  return (
    <>
      {!isOpen && (
        <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} whileHover={{ scale: 1.05 }} transition={iosSpring} onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 w-14 h-14 bg-indigo-600 text-white rounded-full shadow-2xl flex items-center justify-center z-[100]">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed z-[110] bg-white/90 dark:bg-slate-950/90 backdrop-blur-3xl flex flex-col overflow-hidden shadow-3xl ${isFullScreen ? 'inset-0' : 'bottom-6 right-6 w-[400px] h-[600px] rounded-[32px] border border-white/20 dark:border-white/5'}`}
          >
            <div className="bg-indigo-600 p-5 text-white flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setIsFullScreen(!isFullScreen)} className="p-2 hover:bg-white/10 rounded-lg">{isFullScreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-4 rounded-[22px] text-sm leading-relaxed max-w-[85%] ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-900 border border-gray-100 dark:border-white/5 text-gray-800 dark:text-gray-200 rounded-tl-none shadow-sm'}`}>
              ...
              {isLoading && <div className="text-xs font-black text-gray-400 flex items-center gap-2"><Loader2 size={12} className="animate-spin" /> Analyzing data...</div>}
            </div>

            <div className="p-6 border-t border-gray-100 dark:border-white/5">
              <form onSubmit={e => { e.preventDefault(); handleSendMessage(); }} className="relative">
                <input type="text" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Ask anything..." className="w-full bg-gray-50 dark:bg-slate-900 border-none rounded-2xl pl-5 pr-12 py-4 text-sm focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white" />
